...
如果对应供应商的 API Key 存在，脚本会调用模型 API 生成中文标题、今日重点、分领域摘要和简短中文摘要。如果没有配置 API Key，脚本会自动使用本地规则模板生成标题和 fallback summaries，不会因为缺少 Key 直接崩溃。化学标题会优先突出研究对象、方法、材料/反应体系、机制或表征亮点，避免营销号式反问和悬念表达。
...
- 默认本地运行时，如果 SMTP 未配置或 PDF 转换失败，脚本只会记录 `Email not sent`，不会影响 Word 生成。
- 线上 `schedule` / `repository_dispatch` 会使用 `--require-email`，只要 SMTP、收件人、PDF 转换或发信失败，workflow 就会失败，避免误判为已发送。
...
- 线上触发会要求三份日报都成功完成邮件发送；任一学科缺少收件人、SMTP 失败或 PDF 转换失败，workflow 会以失败状态结束。
...
- 只运行 `python main.py --profile chemistry ... --require-email`。
