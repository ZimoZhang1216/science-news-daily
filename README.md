...
先填写姓名、收件邮箱和一段研究兴趣描述，再由面板生成可编辑的研究画像与计划建议；保存画像后只会创建预览任务。预览会生成并上传 DOCX artifact，绝不会向收件人发邮件；PDF 转换仅在确认后的正式定时投递中执行。确认预览内容后，点击“启用固定频率计划”才会启用计划；系统会重新计算严格晚于启用时刻的下一次固定发送时间，并由统一 `Cronjob Daily Research News` workflow 的 30 分钟唤醒在该时间后自动发送。
