...
这个项目会从 arXiv、PubMed、Crossref 期刊元数据和 RSS 源检索近 24 小时到 3 天内的前沿论文/科研资讯，调用 OpenAI 或 DeepSeek 生成中文标题、简短中文摘要和分领域摘要，并输出 Word 文档。化学日报使用严谨的学术亮点标题，生物和统计学保留各自配置的标题风格。当前支持化学、生物、统计学三套日报；配置 SMTP 后，本地仍保存 `.docx`，邮件附件会自动转换为 `.pdf`。
...
如果对应供应商的 API Key 存在，脚本会调用模型 API 生成中文标题、今日重点、分领域摘要和简短中文摘要。如果没有配置 API Key，默认本地运行会自动使用规则模板生成标题和 fallback summaries，不会因为缺少 Key 直接崩溃。化学标题会优先突出研究对象、方法、材料/反应体系、机制或表征亮点，避免营销号式反问和悬念表达。
...
- 线上 `schedule` / `repository_dispatch` 会同时使用 `--require-email`，只要 SMTP、收件人、PDF 转换或发信失败，workflow 就会失败，避免误判为已发送。
...
```cron
0 8 * * * cd "/Users/zhangzimo/Library/Mobile Documents/com~apple~CloudDocs/chem-news-daily" && /bin/zsh -lc 'source .venv/bin/activate && python main.py --profile chemistry >> "./output/run.log" 2>&1 && python main.py --profile biology >> "./output/run.log" 2>&1 && python main.py --profile statistics >> "./output/run.log" 2>&1'
```
...
workflow 使用当天日期 marker 防止 GitHub 备用 `schedule` 重复发送。线上触发规则如下：

- `repository_dispatch` 和 `schedule` 永远强制运行 `chemistry`、`biology`、`statistics` 三份日报，即使外部定时器误传了 `profiles: chemistry` 也会被忽略。
- 线上触发会要求三份日报都成功完成 AI 总结和邮件发送；任一学科缺少模型 API、AI 输出不完整、收件人缺失、SMTP 失败或 PDF 转换失败，workflow 会以失败状态结束。
- 如果当天已经由外部定时器成功执行，后续 GitHub `schedule` 备用触发会自动跳过，避免重复发送。
- 手动 `workflow_dispatch` 仍可用 `profiles` 输入指定单个或多个学科，便于临时测试。

项目还包含 `.github/workflows/chemistry-manual.yml`，这是独立的化学日报手动发送 workflow：

- 只支持 `workflow_dispatch`，没有 `schedule`，不会被 cron-job.org 或 GitHub 定时器触发。
- 只运行 `python main.py --profile chemistry ... --require-email --require-ai`。
- 只检查并使用 `CHEM_REPORT_EMAIL_TO`；如果它为空，则回落到 `REPORT_EMAIL_TO`。
- 不运行生物和统计学日报，不要求 `BIO_REPORT_EMAIL_TO` 或 `STAT_REPORT_EMAIL_TO`。
- 会上传化学日报 `.docx` 和邮件用 `.pdf` artifact。

私人使用场景还包含 `.github/workflows/personal-chem-bio.yml`：

- 只支持 `workflow_dispatch`，没有 `schedule` 和 `repository_dispatch`。
- 手动触发后只运行化学和生物两份日报。
- 两份日报都只发送到 `PERSONAL_REPORT_EMAIL_TO`，不会使用常规的 `CHEM_REPORT_EMAIL_TO` 或 `BIO_REPORT_EMAIL_TO` 收件人。
- 两份日报都必须成功调用模型 API 生成总结，否则不会发送邮件。
- 需要新增 GitHub Secret：`PERSONAL_REPORT_EMAIL_TO`，值填你自己的邮箱；多个私人邮箱也可以用英文逗号或分号分隔。
- 会上传化学和生物日报的 `.docx` 与邮件用 `.pdf` artifact。
...
- `PERSONAL_REPORT_EMAIL_TO`：私人手动 workflow 专用收件人，只用于 `.github/workflows/personal-chem-bio.yml`。
...
外部定时器不需要传 `profiles`；线上 workflow 会自动运行化学、生物、统计学三份日报并发送三封 PDF 附件邮件。
...
2. 在页面底部找到 `Artifacts`。
3. 下载 `research-news-daily-output`。
