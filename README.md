...
- 监听 `repository_dispatch` 的 `event_type=daily-research-news`，也保留 `workflow_dispatch` 便于手动测试。
...
2. 在页面底部找到 `Artifacts`。
3. 下载对应 artifact，例如 `chemistry-target-output`、`biology-personal-output` 或 `cronjob-daily-research-news-output`。
