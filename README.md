...
Word 输出会保留常见科学记号的上下标：HTML `<sub>/<sup>`、化学式中的数字下标（如 `H2O`、`CO2`、`Na2SO4`）、离子电荷（如 `Fe3+`、`NH4+`）以及数学标记（如 `R^2`、`x_i`、`a^n`）会写成真正的 Word 上/下标。配置模型 API 后，脚本还会增加一轮 AI 科学排版校对，只允许模型插入 `^{...}` / `_{...}` 标记，并会校验去掉标记后的文字必须与原文一致，避免模型改写内容。
