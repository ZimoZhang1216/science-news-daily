...
- The venue catalogue is a versioned, in-repository representation of the official CCF recommended conference catalogue. It stores the CCF tier, venue abbreviation, and DBLP conference path, and is explicitly labelled with the CCF catalogue version/date it represents.
...
- Only CCF A/B/C entries configured in the selected scope are collected. Records that look like workshops, demos, short papers, companion proceedings, or other non-main tracks are excluded when DBLP's proceeding title or record key makes that distinction available. This reflects CCF's stated scope for full/regular conference papers.
...
- The computer-science dashboard form presents a constrained selector whose value applies when the CCF source is enabled: `A`, `A + B` (default), and `A + B + C`.
...
- The CCF catalogue belongs in a focused `personalization/ccf_catalogue.py` module instead of `main.py`, so CCF versioning and venue-path mapping are not spread across the report runtime and dashboard.
